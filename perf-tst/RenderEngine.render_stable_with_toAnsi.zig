...
fn testOutputSize(
    ...
    // INCLUDES toAnsi() call to measure combined render + ANSI conversion performance
    for (0..iterations) |_| {
        output_surface.clearTransparent();
        movy.render.RenderEngine.render(input_surfaces.items, output_surface);
        _ = try output_surface.toAnsi(); // Combined performance test
    }
...
pub fn main() !void {
    ...
    if (write_json) {
        ...
        const test_result = types.TestResult{
            .test_name = "RenderEngine.render_stable_with_toAnsi",
        ...
        const filename = try std.fmt.allocPrint(
            allocator,
            "{s}/RenderEngine.render_stable_with_toAnsi_{s}.json",
            .{ args.output_dir, args.suffix.? },
        );
