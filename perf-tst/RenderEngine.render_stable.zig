...
pub fn main() !void {
    ...
    std.debug.print(
        "=== RenderEngine.render() Stable Performance Test ===\n",
        .{},
    );
    std.debug.print(
    ...
    if (write_json) {
        std.debug.print(
            "JSON output: {s}/{s}_RenderEngine.render_stable.json\n\n",
    ...
    try results.append(
        allocator,
        try testOutputSize(allocator, "64x64", 64, 64, iterations),
    );
    try results.append(
        allocator,
        try testOutputSize(allocator, "96x96", 96, 96, iterations),
    );
    try results.append(
        allocator,
        try testOutputSize(allocator, "128x128", 128, 128, iterations),
    );
    try results.append(
        allocator,
        try testOutputSize(allocator, "160x160", 160, 160, iterations),
    );
    try results.append(
        allocator,
        try testOutputSize(allocator, "192x192", 192, 192, iterations),
    );
    try results.append(
        allocator,
        try testOutputSize(allocator, "256x256", 256, 256, iterations),
    );
